...
#
# Version ConozcoNumeros-2 ana.cichero@gmail.com 
# Esta version cambia valor de constantes iniciales
# Incluye archivos nave*.png 
# Incluye las funciones relativas a  la imagen nave.
# 
...
class ConozcoUy():
    ...
    def cargarNiveles(self):
        ...
        while linea:
            ...
            if var.startswith(_("Prefijo")):
                self.listaPrefijos.append(
                    unicode(valor.strip(),'iso-8859-1'))
            elif var.startswith(_("Sufijo")):
                self.listaSufijos.append(
                    unicode(valor.strip(),'iso-8859-1'))
            elif var.startswith(_("Correcto")):
                self.listaCorrecto.append(
                    unicode(valor.strip(),'iso-8859-1'))
            elif var.startswith(_("Mal")):
                self.listaMal.append(
                    unicode(valor.strip(),'iso-8859-1'))
            elif var.startswith(_("Despedida")):
                self.listaDespedidas.append(
                    unicode(valor.strip(),'iso-8859-1'))
            elif var.startswith(_("Pregunta")):
    ...
    def pantallaAcercaDe(self):
        ...
        self.mostrarTexto(_("Acerca de Conozco Numeros"),
                          self.fuente40,
    ...
    def pantallaInicial(self):
        ...
        self.pantalla.fill((0,0,0))
        self.mostrarTexto(_("CONOZCO NUMEROS"),
                          self.fuente48,
                          (int(600*scale+shift_x),
